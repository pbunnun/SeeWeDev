...
/**
 * @file ActivateAllNodesModel.hpp
 * @brief Interactive button node for manual triggering and control.
 *
 * This file defines the ActivateAllNodesModel class, which embeds a QPushButton widget
 ...
 * ActivateAllNodes → [sync] → CameraCapture → [image] → Display
 * 
 * // Click counter for testing
 * ActivateAllNodes → [int] → LCDNumber (shows click count)
 * 
 * // Manual save trigger
 * ActivateAllNodes → [sync] → SaveImage (saves current frame)
 * 
 * // Step-through processing
 * ActivateAllNodes → [sync] → ProcessingNode → NextNode
 * @endcode
 *
 * **Output Behavior:**
 ...
 * - Embedded QActivateAllNodes widget
 * - Persistent click count during session
 *
 * **Design Patterns:**
 * @code
 * // Gate pattern - enable/disable processing
 * ActivateAllNodes → [sync] → SyncGate → ProcessingChain
 * 
 * // Trigger pattern - one-shot operations
 * ActivateAllNodes → [sync] → OneShot → Action
 * 
 * // Counter pattern - limited executions
 * ActivateAllNodes → [int] → Threshold → Conditional
 * @endcode
 *
...
/**
 * @class ActivateAllNodesModel
 * @brief Interactive push button node for manual triggering.
 *
 * This node embeds a QActivateAllNodes widget to provide user-initiated triggers
 ...
 *
 * **Widget Properties:**
 * - **mpEmbeddedWidget:** Embedded QActivateAllNodes widget
 ...
 * ActivateAllNodes → [sync] → CameraCaptureNode → [image] → Display
 * 
 * // Click counter display
 * ActivateAllNodes → [int] → LCDNumberModel (shows total clicks)
 * 
 * // Conditional execution
 * ActivateAllNodes → [int] → MathCondition (execute every 5 clicks)
 * 
 * // Dual control
 * ActivateAllNodes → [sync] → Action1
 *           → [int]  → Counter → Threshold → Action2
 * @endcode
 *
 * **Testing Workflows:**
 * Use ActivateAllNodes for step-by-step debugging:
 * @code
 * // Step through image processing
 * ActivateAllNodes → [sync] → LoadImage → Filter1 → Filter2 → Display
 * 
 * // Manual frame advance
 * ActivateAllNodes → [sync] → VideoCapture (advances one frame)
 * 
 * // Iterative testing
 * ActivateAllNodes → [sync] → Algorithm → ResultValidator
 * @endcode
 *
 ...
 */
class ActivateAllNodesModel : public PBNodeDelegateModel
{
    Q_OBJECT
...
public:
     ...
     *
     * Initializes the QActivateAllNodes widget and connects the clicked()
     ...
     */
    virtual
        ~ActivateAllNodesModel() override { }
     ...
     * @return 0 for both In (no inputs) and Out (no outputs).
     */
    unsigned int
    nPorts(PortType) const override { return 0; };

    /**
     * @brief Returns the data type for a specific port.
     * @param portType Input or Output
     * @param portIndex Port index
     * @return NodeDataType for the specified port (none for this node : TODO)
     */
    NodeDataType
...
private Q_SLOTS:
    ...
    * 3. Call emitOutputPort(0) for sync port
    * 4. Call emitOutputPort(1) for count port
     */
    void
