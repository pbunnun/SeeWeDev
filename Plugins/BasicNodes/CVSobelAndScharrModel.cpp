...
const QString CVSobelAndScharrModel::_category = QString( "Image Processing" );

const QString CVSobelAndScharrModel::_model_name = QString( "CV Sobel and Scharr" );

CVSobelAndScharrModel::

CVSobelAndScharrModel()
    : PBNodeDelegateModel( _model_name ),
      mpEmbeddedWidget(new CVSobelAndScharrEmbeddedWidget()),
...
void CVSobelAndScharrModel::em_checkbox_checked(int)
{
    if(mpCVImageInData)
    {
        processData(mpCVImageInData,mapCVImageData,mParams);
...
void CVSobelAndScharrModel::processData(const std::shared_ptr<CVImageData> &in, std::shared_ptr<CVImageData> (&out)[3],
