...
#include "CVRGBsetValueModel.hpp" //INCOMPLETE
...
const QString CVRGBsetValueModel::_category = QString("Image Conversion");

const QString CVRGBsetValueModel::_model_name = QString("CV RGB Values");
...
unsigned int CVRGBsetValueModel::nPorts(PortType portType) const
{
...
NodeDataType CVRGBsetValueModel::dataType(PortType,PortIndex) const
{
    return CVImageData().type();
}

std::shared_ptr<NodeData> CVRGBsetValueModel::outData(PortIndex)
{
    if( isEnable() )
    {
...
void CVRGBsetValueModel::setInData(std::shared_ptr<NodeData> nodeData, PortIndex)
{
    if(nodeData)
    {
...
void CVRGBsetValueModel::load(QJsonObject const &p)
{
    /*
...
em_button_clicked( int button )
{
    ...
    DEBUG_LOG_INFO() << "[em_button_clicked] Apply RGB values";
    if(mpCVImageInData)
    {
        mpCVImageData->set_image(mpCVImageInData->data());
        Q_EMIT dataUpdated( 0 );
...
void CVRGBsetValueModel::setModelProperty(QString &id, const QVariant & value)
{
...
void CVRGBsetValueModel::processData(std::shared_ptr<CVImageData> &out, const CVRGBsetValueProperties &props)
{
