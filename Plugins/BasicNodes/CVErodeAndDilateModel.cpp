...
createWorker()
{
    return new CVErodeAndDilateWorker();
}
...
{
    auto* w = qobject_cast<CVErodeAndDilateWorker*>(worker);
    if (w) {
        connect(w, &CVErodeAndDilateWorker::frameReady,
...
void CVErodeAndDilateModel::process_cached_input()
{
