...
/**
 * @file CVRTSPCameraModel.hpp
 * @brief Camera capture node for real-time image acquisition from RTSP streams.
 *
 * This file defines the CVRTSPCameraModel node which provides live camera feed integration
 ...
 * ```
 * CVRTSPCamera → CVImageDisplay        (Live preview)
 * CVRTSPCamera → FaceDetection → Draw  (Real-time detection)
 * CVRTSPCamera → Threshold → FindContour (Object tracking)
 * Timer → CVRTSPCamera → Processing     (Triggered capture at fixed rate)
 * ```
 *
 * **Common Applications:**
...
// Forward declaration
class CVRTSPCameraModel;

/**
 * @struct CVRTSPCameraParameters
 ...
 */
typedef struct CVRTSPCameraParameters{
    ...
    int miBufferSize {1};           ///< Buffer size (1=minimal latency, 10=more buffering)
 ...
 * @brief Async worker (QObject) for RTSP camera capture inside PBAsyncDataModel worker thread.
 */
class CVRTSPCameraWorker : public QObject
{
    Q_OBJECT
public:
    explicit CVRTSPCameraWorker(QObject *parent = nullptr);

public Q_SLOTS:
    void setRtspUrl(const QString& url);
    void setParams(CVRTSPCameraParameters params);
...
private:
    ...
    double mdFPS{0};
    CVRTSPCameraParameters mRTSPCameraParams;
 ...
 *
 * CVRTSPCameraModel provides real-time image acquisition from USB, built-in, or V4L2 cameras.
 ...
 * 1. **Live Preview:**
 *    ```
 *    CVRTSPCamera → CVImageDisplay
 *    ```
 *
 * 2. **Real-Time Object Detection:**
 *    ```
 *    CVRTSPCamera → FaceDetection → DrawContour → CVImageDisplay
 *    CVRTSPCamera → HoughCircleTransform → DrawContour → CVImageDisplay
 *    ```
 *
 * 3. **Fixed-Rate Processing (avoid camera FPS variations):**
 *    ```
 *    Timer(10Hz) → CVRTSPCamera → Threshold → FindContour → Analysis
 *    ```
 *
 * 4. **Multi-Camera Synchronization:**
 *    ```
 *    Timer → CombineSync ┬→ CVRTSPCamera(ID=0) → Process0
 *                         ├→ CVRTSPCamera(ID=1) → Process1
 *                         └→ CVRTSPCamera(ID=2) → Process2
 *    ```
 *
 * 5. **Conditional Capture (only when object detected):**
 *    ```
 *    CVRTSPCamera(continuous) → MotionDetect → [if motion] → Trigger → CVRTSPCamera(single-shot) → SaveImage
 *    ```
 *
 * 6. **Barcode Scanner:**
 *    ```
 *    CVRTSPCamera → Threshold → FindContour → BarcodeDecoder → Display
 *    ```
 *
 ...
 */
class CVRTSPCameraModel : public PBAsyncDataModel
{
    Q_OBJECT
...
private:
private:
    // RTSP latency reduction settings
    int miBufferSize {1};           ///< Buffer size (1 = minimal for low latency)
    int miConnectTimeoutMs {5000};  ///< Connection timeout milliseconds
    int miReadTimeoutMs {5000};     ///< Read timeout milliseconds

    // RTSP streams come pre-encoded - no client-side parameter control needed
    CVRTSPCameraParameters mRTSPCameraParams; ///< Basic parameters for internal use
    bool mAutoRefreshAllowed {true}; ///< Allow automatic operations

    CVRTSPCameraProperty mCameraProperty;                   ///< RTSP URL and status
    CVRTSPCameraEmbeddedWidget * mpEmbeddedWidget;          ///< UI controls

    CVRTSPCameraWorker * mpCameraWorker { nullptr };        ///< Async capture worker (in PBAsync thread)

    std::shared_ptr< SyncData > mpSyncInData { nullptr };       ///< Trigger input (single-shot mode)
    std::shared_ptr< SyncData > mpSyncData { nullptr };         ///< Sync output signal
    ...
    double mdLastFps{0.0};                                      ///< Cached FPS from worker
